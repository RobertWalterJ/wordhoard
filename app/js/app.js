...
const ICON = {
  speak: 'M3 6.2h2.6L9 3.3v9.4L5.6 9.8H3zM11.4 5.4a3.6 3.6 0 0 1 0 5.2M13.3 3.4a6.2 6.2 0 0 1 0 9.2',
  star: 'M8 1.9l1.75 3.9 4.05.45-3 2.85.85 4.15L8 11.3l-3.65 1.95.85-4.15-3-2.85 4.05-.45z',
};
...
function renderHome() {
  ...
  if (fit.trials < 12) {
    ...
    card.append(
      el('span', 'eyebrow', 'Your vocabulary'),
      num, bandWrap,
      el('span', 'est-range', `${n0(v.lo)} – ${n0(v.hi)} · from ${n0(fit.trials)} answers`),
  ...
  for (const m of MODES) {
    ...
    b.type = 'button';
    const left = el('span', 'mode-body');
    left.append(el('span', 'mode-name', m.name), el('span', 'mode-desc', m.desc));
    let tag = m.tag;
    if (m.id === 'train') {
    ...
    b.append(left, el('span', 'mode-tag', tag));
...
function nextQuestion() {
  ...
  if (session.mode === 'rapid') {
    ...
    chip.hidden = false;
    chip.textContent = `${session.score.toLocaleString('en-CA')} points${session.streak > 1 ? ` · ${session.streak} in a row` : ''}`;
  } else {
